...
public abstract class ClassUtils {
	...
	public static Class<?> forName(String name, ClassLoader classLoader) throws ClassNotFoundException, LinkageError {
		...
		{
			resolvedClassCache = new ConcurrentReferenceHashMap();
			classResolverCache.put(classLoader, resolvedClassCache);
		}
		Object cached = resolvedClassCache.get(name);
		if (cached != null)
		{
		...
		try {
			Class<? extends Object> result = clToUse != null ? clToUse.loadClass(name) : Class.forName(name);
			resolvedClassCache.put(name, result);
			return result;
		}
			...
			if (lastDotIndex != -1) {
				...
				try {
					Class<? extends Object> result = clToUse != null ? clToUse.loadClass(innerClassName) : Class.forName(innerClassName);
					resolvedClassCache.put(name, result);
					return result;
				}
			...
			resolvedClassCache.put(name, NULL_VALUE);
			throw ex;
		}
	}
